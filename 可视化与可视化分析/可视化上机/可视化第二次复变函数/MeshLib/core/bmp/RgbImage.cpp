...
{  
	...
	for ( int i=0; i<NumRows; i++ ) {
		int j;
		for ( j=0; j<NumCols; j++ ) {
			*(cPtr+0) = fgetc( infile );	// Blue color value
			*(cPtr+1) = fgetc( infile );	// Green color value
			*(cPtr+2) = fgetc( infile );		// Red color value
			//*(cPtr+0) = 255;	// Blue color value
			//*(cPtr+1) = 255;	// Green color value
			//*(cPtr+2) = 255;		// Red color value
			cPtr += 3;
		}
